...
from models import MLP,  CNN,CNN2D,RFSignalCNN,ResNet18,ResNet34
...
from N_pair_loss import NpairLoss ,npair_loss
...
if __name__ == '__main__':
    ...
    #loss_func = NpairLoss(l2_reg=0.02)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (data, labels) in enumerate(trainloader):
            ...
            # 初始化正负样本 PyTorch 张量
            positive_samples = torch.empty((data.size(0), *data.shape[1:]), dtype=torch.float32).to(device)
            negative_samples = torch.empty((data.size(0), *data.shape[1:]), dtype=torch.float32).to(device)
            ...
            positive_features, positive_outputs = global_model(positive_samples)
            negative_features, negative_outputs = global_model(negative_samples)

            loss = loss_func(anchor_features, positive_features, negative_features, outputs, labels)
            #loss = loss_func(anchor_features, positive_features, labels)
            loss.backward()
            optimizer.step()
            # 累计损失
    ...
    #保存完整模型（结构+权重）
    model_path = '../save/TripletCNN/centralized_Tripletmodel_{}_{}_{}_Triplet_{}_{}_lr[{}].pth'.format(args.model,args.data_choice, args.epochs,  args.process,args.datasettype,
                args.lr)
    ...
    # 定义保存的文件路径
    file_name = '../save/TripletCNN/{}_{}_{}_Triplet_{}_{}_lr[{}].txt'. \
    ...
    def visualize_features_with_tsne(features, labels, class_labels):
        ...
        # 添加颜色条（可选）
        plt.colorbar(scatter, ticks=range(25), label="Class")
        ax.set_title("t-SNE Visualization of Feature Vectors (3D)", fontsize=16)
